...
/**
 * Transformer: AUMOVIO Dynamic Media / Scene7 images.
 *
 * Source images are Adobe Scene7 / Dynamic Media assets (URLs contain
 * /is/image/), verified in migration-work/metadata.json .images.mapping keys
 * (e.g. https://aumovio.scene7.com/is/image/aumovio/...).
 *
 * Rewrites every DM/S7 <img> into an anchor so the URL round-trips through
 * markdown intact; the companion auto-block in scripts/scripts.js rebuilds
 * responsive <picture> elements client-side. Runs in afterTransform ONLY so
 * block parsers can extract <img> references into their cells first.
 *
 * See sub-agents/excat-import-infrastructure/references/dm-scene7-transformer.md
 * for the canonical spec. Inlined helpers below are kept byte-identical with
 * references/dm-scene7-helpers.js.
 */
